...
def run(materialName="TempMat", channel_dict= None , render_plugin="Arnold"):
    '''
    ...
    for shd in pm.ls(materials=True):
        if [c for c in shd.classification() if 'shader/surface' in c and materialName in str(shd)]:
            shader_exists =True
            target_shader = str(shd)
        elif [c for c in shd.classification() if 'shader/surface' in c and materialName not in str(shd)]:
            pass
    if shader_exists:
        print "=== Shader Exists ===\n=== Update Mode ==="
        ...
        create_network(target_obj_list,materialName,channel_dict,render_plugin)

def create_network(target_obj_list,materialName,channel_dict,render_plugin):
    print "==== Start Creating %s Network ===="%render_plugin
    #create a shader and assign to selection
    current_shader=shadingNode(render_plugin_dict[render_plugin],name=(materialName),asShader=1,skipSelect=1)
...
def update_textures(current_shader,channel_dict,render_plugin,current_SG):
    ...
    for param_name in channel_dict:
        ...
        if pm.objExists(utils_node):
            ...
            file_node = updateFileTexture(0,utils_node,utils_node+'_p2d', texture_path, texture_color_space, render_plugin)
            ...
            param_name = 'input'
            file_node = updateFileTexture(1, utils_node, utils_node+'_p2d', texture_path, texture_color_space, render_plugin)
...
def updateFileTexture(create_file,file_name, p2d_name,texture_path,texture_color_space,render_plugin):
